          ...
          <div style={{ marginBottom: 20, position: 'relative' }}>
            ...
            {showCustDropdown && filteredCustomers.length > 0 && (
              <div style={{
                ...
                boxShadow: '0 8px 16px rgba(146, 64, 14, 0.15)'
              }}>
          ...
          <div className="modal-box" style={{ maxWidth: 840 }}>
            ...
            <div className="modal-body" style={{ background: '#fffdf5', color: '#0f172a', padding: 36, fontFamily: 'serif' }}>
              <div id="invoice-preview">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', border: '1px solid #b45309', padding: 12, marginBottom: 20, fontSize: 12 }}>
                  <div>
                ...
                {/* Items Table */}
                <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 20, fontSize: 12, border: '1px solid #b45309' }}>
                  <thead>
                    <tr style={{ background: '#fef3c7', borderBottom: '1px solid #b45309' }}>
                      <th style={{ border: '1px solid #b45309', padding: 6, width: 30 }}>S.N</th>
                      <th style={{ border: '1px solid #b45309', padding: 6, textAlign: 'left' }}>Description of Ornaments</th>
                      <th style={{ border: '1px solid #b45309', padding: 6, width: 60 }}>HSN</th>
                      <th style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right', width: 70 }}>Net Wt (g)</th>
                      <th style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right', width: 60 }}>Wastage %</th>
                      <th style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right', width: 70 }}>Gross Wt</th>
                      <th style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right', width: 70 }}>Rate/g</th>
                      <th style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right', width: 70 }}>Making (₹)</th>
                      <th style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right', width: 90 }}>Amount (₹)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {invoiceData.items.map((item, index) => {
                      ...
                      return (
                        <tr key={index}>
                          <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'center' }}>{index + 1}</td>
                          <td style={{ border: '1px solid #b45309', padding: 6 }}>{item.description}</td>
                          <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'center' }}>{item.hsnCode}</td>
                          <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right' }}>{Number(item.netWeight).toFixed(3)}g</td>
                          <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right' }}>{item.wastagePct}%</td>
                          <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right' }}>{gross.toFixed(3)}g</td>
                          <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right' }}>₹{Number(item.rate).toLocaleString('en-IN')}</td>
                          <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right' }}>₹{Number(item.makingCharges).toLocaleString('en-IN')}</td>
                          <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right' }}>₹{Number(item.amount).toLocaleString('en-IN')}</td>
                        </tr>
                      );
                    })}
                    {Number(invoiceData.otherCharges) > 0 && (
                      <tr>
                        <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'center' }}>{invoiceData.items.length + 1}</td>
                        <td style={{ border: '1px solid #b45309', padding: 6 }} colSpan="7">Other Charges / Additions</td>
                        <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right' }}>₹{Number(invoiceData.otherCharges).toLocaleString('en-IN')}</td>
                      </tr>
                    )}
                    <tr style={{ fontWeight: 'bold' }}>
                      <td colSpan="3" style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right' }}>Total Weight &amp; Subtotal:</td>
                      <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right' }}>
                        {invoiceData.items.reduce((sum, i) => sum + (Number(i.netWeight) || 0), 0).toFixed(3)}g
                      </td>
                      <td style={{ border: '1px solid #b45309' }}></td>
                      <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right' }}>
                        {invoiceData.items.reduce((sum, i) => sum + (Number(i.netWeight) * (1 + (Number(i.wastagePct) || 0) / 100)), 0).toFixed(3)}g
                      </td>
                      <td colSpan="2" style={{ border: '1px solid #b45309' }}></td>
                      <td style={{ border: '1px solid #b45309', padding: 6, textAlign: 'right' }}>₹{subTotal.toLocaleString('en-IN')}</td>
                    </tr>
                  </tbody>
                </table>
                ...
                <div style={{ display: 'flex', gap: 14, marginBottom: 20, fontSize: 12 }}>
                  {/* Words */}
                  <div style={{ flex: 1, border: '1px solid #b45309', padding: 10 }}>
                  ...
                  {/* Tax summary table */}
                  <div style={{ width: 320 }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #b45309', fontSize: 11 }}>
                      <tbody>
                        <tr>
                          <td style={{ border: '1px solid #b45309', padding: 4 }}>Subtotal Taxable</td>
                          <td style={{ border: '1px solid #b45309', padding: 4, textAlign: 'right' }}>₹{subTotal.toFixed(2)}</td>
                        </tr>
                        <tr>
                          <td style={{ border: '1px solid #b45309', padding: 4 }}>CGST ({invoiceData.cgstRate}%)</td>
                          <td style={{ border: '1px solid #b45309', padding: 4, textAlign: 'right' }}>₹{cgstAmount.toFixed(2)}</td>
                        </tr>
                        <tr>
                          <td style={{ border: '1px solid #b45309', padding: 4 }}>SGST ({invoiceData.sgstRate}%)</td>
                          <td style={{ border: '1px solid #b45309', padding: 4, textAlign: 'right' }}>₹{sgstAmount.toFixed(2)}</td>
                        </tr>
                        <tr style={{ fontWeight: 'bold', background: '#fef3c7' }}>
                          <td style={{ border: '1px solid #b45309', padding: 4 }}>Grand Total</td>
                          <td style={{ border: '1px solid #b45309', padding: 4, textAlign: 'right' }}>₹{grandTotal.toFixed(2)}</td>
                        </tr>
                        {invoiceData.oldGoldAmount > 0 && (
                          <>
                            <tr>
                              <td style={{ border: '1px solid #b45309', padding: 4, color: '#c00' }}>Old Gold Exchange (Deduction)</td>
                              <td style={{ border: '1px solid #b45309', padding: 4, textAlign: 'right', color: '#c00' }}>
                                - ₹{Number(invoiceData.oldGoldAmount).toFixed(2)}
                              </td>
                            </tr>
                            <tr style={{ fontWeight: 'bold', background: '#fef3c7', fontSize: 12 }}>
                              <td style={{ border: '1px solid #b45309', padding: 4 }}>Final Payable Amount</td>
                              <td style={{ border: '1px solid #b45309', padding: 4, textAlign: 'right' }}>₹{finalAmount.toLocaleString('en-IN')}</td>
                            </tr>
                          </>
                        )}
                      </tbody>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 36, fontSize: 12 }}>
                  ...
                  <div style={{ textAlign: 'right', width: '30%' }}>
                    ...
                    <p style={{ marginTop: 40, borderTop: '1px solid #b45309', display: 'inline-block', width: 140, paddingTop: 4 }}>Authorised Signatory</p>
                  </div>
                </div>
              </div>
